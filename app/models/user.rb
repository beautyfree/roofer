class User
  ...
  ROLES = %w[banned guest user moderator admin]
  ...
  field :role_mask, type: Integer, default: 1
  attr_accessible :nickname, :provider, :url, :username

  has_many :transactions
  has_and_belongs_to_many :roofs

  recommends :roofs

  def balance
    t = transactions.order_by(:created_at => :asc).last
    if t
      t.balance
    else
      0
  ...
  def role?(base_role)
    ROLES.index(base_role.to_s) <= role_mask
  end
  ...
  before_create do
    self.role_mask = ROLES.index(:roofer)
  end
